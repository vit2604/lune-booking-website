...
export function validatePhoneNumber({ phoneCode, phoneNumber, country } = {}) {
  const rawNumber = String(phoneNumber || '').trim();
  if (!rawNumber || !/^[0-9+\-().\s]+$/.test(rawNumber)) return false;

  const selectedCallingCode = String(phoneCode || '').match(/^\+\d+/)?.[0];
  const countryCode = resolveCountry(phoneCode, country);
  if (!selectedCallingCode && !rawNumber.startsWith('+')) return false;
  ...
  return !selectedCallingCode || `+${parsed.countryCallingCode}` === selectedCallingCode;
}
